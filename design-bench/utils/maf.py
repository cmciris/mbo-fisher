...
def fetch_dataloaders(dataset_name, batch_size, device, flip_toy_var_order=False, toy_train_size=25000, toy_test_size=5000):
    ...
    # imaging dataset pulled from torchvision
        ...
        train_dataset = load_dataset(dataset_name)(root='data/', train=True, transform=image_transforms, target_transform=target_transforms)
        test_dataset =  load_dataset(dataset_name)(root='data/', train=True, transform=image_transforms, target_transform=target_transforms)
...
class BatchNorm(nn.Module):
    ...
    def forward(self, x, cond_y=None):
        ...
        # print('in sum log var {:6.3f} ; out sum log var {:6.3f}; sum log det {:8.3f}; mean log_gamma {:5.3f}; mean beta {:5.3f}'.format((var + self.eps).log().sum().data.numpy(), y.var(0).log().sum().data.numpy(), log_abs_det_jacobian.mean(0).item(), self.log_gamma.mean(), self.beta.mean()))
...
def build_density_model(input_shape, activation_fn, hidden_size, n_hidden=2, algo='made', cond_label_size=None, input_order='sequential', n_components=1, n_blocks=5, batch_norm=True):
    input_size = np.prod(input_shape)
    # model
    if algo == 'made':
        model = MADE(input_size, hidden_size, n_hidden, cond_label_size, activation_fn, input_order)
    elif algo == 'mademog':
        assert n_components > 1, 'Specify more than 1 component for mixture of gaussians models.'
        model = MADEMOG(n_components, input_size, hidden_size, n_hidden, cond_label_size, activation_fn, input_order)
    elif algo == 'maf':
        model = MAF(n_blocks, input_size, hidden_size, n_hidden, cond_label_size, activation_fn, input_order, batch_norm=batch_norm)
    elif algo == 'mafmog':
        assert n_components > 1, 'Specify more than 1 component for mixture of gaussians models.'
        model = MAFMOG(n_blocks, n_components, input_size, hidden_size, n_hidden, cond_label_size, activation_fn, input_order, batch_norm=batch_norm)
    elif algo =='realnvp':
        model = RealNVP(n_blocks, input_size, hidden_size, n_hidden, cond_label_size, batch_norm=batch_norm)
    else:
...
class DensityTrainer():
    def __init__(self,
        ...
        self.optimizer = density_model_optim
        self.lr = density_model_lr
    ...
    def get_optimizer(self, parameters):
        if self.optimizer == 'Adam':
            return torch.optim.Adam(parameters, lr=self.lr, weight_decay=self.weight_decay,
                              betas=(self.beta1, 0.999))
        elif self.optimizer == 'RMSProp':
            return torch.optim.RMSprop(parameters, lr=self.lr, weight_decay=self.weight_decay)
