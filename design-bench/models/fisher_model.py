...
class Fisher(nn.Module):
    ...
    def __init__(self,
                 ...
                 forward_model_beta=1.0,
                 opt_limit=None,
                 particle_lr=0.05,
                 entropy_coefficient=0.9,
                 f_coefficient=1.0,
                 noise_std=0.0,
                 model_dir="checkpoints/fisher",
                 model_load=False):
        ...
        self.forward_model_beta = forward_model_beta
        self.opt_limit = opt_limit
        self.particle_lr = particle_lr
        self.entropy_coefficient = entropy_coefficient
        self.f_coefficient = f_coefficient
    ...
    def optimize(self, x, steps):
        ...
        def gradient_step(xt):            
            ...
            offset = self.forward_model(xt)

            # the conservatism of the current set of particles
            loss = self.entropy_coefficient * entropy + offset
            
            loss.backward(torch.ones_like(loss), retain_graph=True)
            grad1 = xt.grad

            # the value of dlogmu(x)/dx
            grad2 = self.behavior_model.scaled_score(xt)
            grad = grad1 + self.forward_model_beta * grad2
            ...
            with torch.no_grad():
                ...
                # xt_ = xt + self.particle_lr * (grad / xt)
    ...
    def train_step(self, x, y):
        ...
        x.requires_grad = True  # after noise, make x be a leaf
        statistics = dict()
        
        # calculate the prediction error and accuracy of the model
        offset = self.forward_model(x)  # [bs, 1]
        with torch.no_grad():
        ...
        grad = x.grad
        f_reg = torch.sum(torch.square(grad), dim=-1).unsqueeze(1)  # \|\nable O(x)\|^2 [bs, D] -> [bs,]

        f_reg_loss = torch.mean(mse_loss + self.f_coefficient * f_reg)

        statistics[f"train/mse_loss"] = torch.mean(mse_loss.detach()).unsqueeze(dim=0)
        statistics[f"train/f_reg"] = torch.mean(f_reg.detach()).unsqueeze(dim=0)
        statistics[f"train/nll"] = f_reg_loss.detach().unsqueeze(dim=0)

        # evaluate how correct the rank of the model predictions are
        rank_corr = spearmanr(y.detach().cpu().numpy()[:, 0], pred.detach().cpu().numpy()[:, 0]).correlation
        ...
        total_loss = torch.mean(f_reg_loss) + multiplier_loss
    ...
    def validate_step(self, x, y):
        ...
        # calculate the prediction error and accuracy of the model
        offset = self.forward_model(x)
        with torch.no_grad():
            logmu = self.behavior_model.scaled_log_prob(x)
        pred = offset + self.forward_model_beta * logmu
        mse_loss = F.mse_loss(y, pred)
        statistics[f"validate/nll"] = mse_loss.detach().unsqueeze(dim=0)
