...
class DistanceAggregator(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        k = self.k_net(k)
        v = self.v_net(v)
        q_repeats = torch.repeat_interleave(q, k.shape[1], dim=0).reshape(q.shape[0], -1, q.shape[-1])
        dist = torch.norm(q_repeats - k, p=2, dim=-1)
        weights = F.softmax(dist, dim=-1) # (batch_size, size_retrieval_set)
        return (v * weights.unsqueeze(-1)).sum(dim=1)
...
class RIM(nn.Module):
    def __init__(self,
        ...
        self.fc_layers_1 = build_forward_model(
        ...
        self.fc_layers_2 = build_forward_model(
            input_shape=self.input_size + self.output_size,
            output_shape=output_shape,
            activations=activations,
            hidden=hidden,
            dropout=dropout,
    ...
    def forward(self, target, retrieval_set):
        ...
        output_1 = self.fc_layers_1(target)
        output_2 = self.fc_layers_2(self.rl)
        output = output_0 * self.weights[0] + output_1 * self.weights[1] + output_2 * self.weights[2]
...
class DimensionRIM(RIM):
    def __init__(self,
        ...
        self.fc_layers_2 = build_forward_model(
            input_shape=self.input_size * 2 + self.output_size - self.opt_channel,
            output_shape=output_shape,
            activations=activations,
            hidden=hidden,
            dropout=dropout,
    ...
    def forward(self, target, retrieval_set):
        ...
        output_1 = self.fc_layers_1(self.input[:, :self.opt_channel])
        output_2 = self.fc_layers_2(self.input[:, self.opt_channel:])
        output = output_0 * self.weights[0] + output_1 * self.weights[1] + output_2 * self.weights[2]
        return output, output_1
