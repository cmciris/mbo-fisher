...
class RetrievalInteractionMachine(nn.Module):
    ...
    def search_engine(self, x, pool):
        """
        retrieval_set: [batch_size, size_retrieval_set, x_dim + y_dim]
        """
        if self.retrieval_method == "distance":
            pool_x, pool_y = pool[0], pool[1]
            repeat_x = torch.repeat_interleave(x, repeats=pool_x.shape[0], dim=0)
            repeat_pool = pool_x.repeat(x.shape[0], 1)
            repeat_x -= repeat_pool
            # for idx in range(x.shape[0]):
            #     repeat_x[idx * pool_x.shape[0] : (idx + 1) * pool_x.shape[0]] -= pool_x
            distance = torch.norm(repeat_x, p=2, dim=-1).reshape(x.shape[0], pool_x.shape[0])
            ...
            pool_x, pool_y = pool[0], pool[1]
            inner_product = torch.einsum("ik,jk->ij", x, pool_x)
            x_norm = torch.repeat_interleave(torch.norm(x, p=2, dim=-1).unsqueeze(-1), inner_product.shape[-1], dim=-1)
            pool_norm = torch.repeat_interleave(torch.norm(pool_x, p=2, dim=-1).unsqueeze(0), inner_product.shape[0], dim=0)
            inner_product /= torch.clamp(x_norm * pool_norm, min=1e-8)
            _, indices = torch.sort(inner_product, dim=-1, descending=True)
            indices = indices[:, :self.size_retrieval_set]
            retrieval_set = torch.cat([pool_x[indices], pool_y[indices]], dim=-1)
            return retrieval_set
        else:
            pool_x, pool_y = pool[0], pool[1]
            scores = torch.einsum("ik,jk->ij", x, pool_x)
    ...
    def train_step(self, x, y, pool):
        ...
        d, main_body = self.forward_model(x, retrieval_set)  # [batch_size, output_dim]
        nll = F.mse_loss(y, d) * self.loss_weights[0] + F.mse_loss(y, main_body) * self.loss_weights[1]
        statistics[f"train/nll"] = nll.detach().unsqueeze(dim=0)

        # hinge loss
        batch_size = d.shape[0]
        # target = - torch.sum(torch.var(d.reshape(batch_size, -1, 2), dim=-2), dim=-1, keepdim=True)
        # hinge_loss = torch.max(torch.zeros_like(target), target - y)
    ...
    def validate_step(self, x, y, pool):
        ...
        # calculate the prediction error and accuracy of the model
        d, main_body = self.forward_model(x, retrieval_set)
        nll = F.mse_loss(y, d) * self.loss_weights[0] + F.mse_loss(y, main_body) * self.loss_weights[1]
