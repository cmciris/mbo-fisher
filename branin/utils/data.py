...
def build_pipeline(x, y, val_size=200, train_size=800, batch_size=128, bootstraps=0, bootstraps_noise=None, buffer=None):
    ...
    if bootstraps > 0:
        bootstraps_training_dataset = [BraninDataSet([bootstraps_train_inputs[0][b], bootstraps_train_inputs[1][b]]) for b in range(bootstraps)]
        validation_dataset = BraninDataSet(validate_inputs)
        ...
        training_dataset = BraninDataSet(train_inputs)
        validation_dataset = BraninDataSet(validate_inputs)
...
def build_rim_pipeline(x, y, pool_size=800, val_size=200, train_size=800, batch_size=128, bootstraps=0, bootstraps_noise=None, buffer=None):
    ...
    x = x[indices]
    y = y[indices]
    train_end = val_size + pool_size + train_size if val_size + pool_size + train_size < len(x) else -1
