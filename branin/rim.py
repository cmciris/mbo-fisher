from models.forward_model import build_forward_model, build_oracle, RIM, DimensionRIM
from models.gradient_ascent import GradientAscent
from models.rim_model import RetrievalInteractionMachine
from models.oracle_model import BraninOracle
import torch
...
from utils.data import BraninTask, build_pipeline, build_rim_pipeline
...
def rim(**config):
    ...
    train_loss_weights=config.get('train_loss_weights')
    ...
    pred_x, pred_y = torch.zeros([0, 2], device=device), torch.zeros([0, 1], device=device)
    forward_model.eval()
    with torch.no_grad():
